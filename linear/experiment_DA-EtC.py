...
for k in range(10):# Determine the number of experiments
    ...
    print('naive seed = {},n={},m={}'.format(seed,n,m))
    random.seed(seed)
    np.random.seed(seed)
    # set parameters
    delta0 = 0.95
    ...
    # for i in range(c): 
    #     for j in range(m): # Initialize the parameter to be estimated
    #         H1_prediction[i][j]=1
    ...
    fpath = os.path.join('results', dataset.lower(), 'linear_ETC_naive_sd-'+str(seed)+'')
    # Save the results
