...
for k in range(20):# Determine the number of experiments
    ...
    for t in range(1, T+1):
        ...
        spending[winner] += beta[winner] * v_prediction[winner, j] # winner gets its v_prediction[winner, j]
        # update beta_ave
        ...
        if(np.random.binomial(1,v[winner][j])):
            v_sum[winner][j]+=1
        v_prediction[winner][j]=v_sum[winner][j]/count[winner][j]#update v_prediction
        NSW=1
        #update NSW
        for i in range(n):
    ...
    fpath = os.path.join('results', dataset.lower(), 'greedy_sd-'+str(seed)+'')
